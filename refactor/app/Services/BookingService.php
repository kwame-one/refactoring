...
use DTApi\Repository\Contracts\UserRepositoryRepositoryInterface;
use DTApi\Repository\Contracts\ThrottleRepositoryRepositoryInterface;
use DTApi\Repository\Contracts\TranslatorRepositoryRepositoryInterface;
...
class BookingService implements BookingServiceInterface
{
    ...
    )
    {
        ...
        $this->langugageRepository = $languageRepository;
    ...
    {
        return $this->repository->query()->with(['translatorJobRel.user'])->find($jobId);
    }
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobs = $this->repository->getTranslatorJobs($cuser->id, 'new')->pluck('jobs')->all(); 
            $usertype = 'translator';
        }
        if ($jobs) {
            ...
            $noramlJobs = collect($noramlJobs)->each(function ($item, $key) use ($user_id) {
                $item['usercheck'] = $this->repository->checkParticularJob($user_id, $item);
            })->sortBy('due')->all();
        }
    ...
    public function getUsersJobsHistory($user_id, array $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobs_ids = $this->repository->getTranslatorJobsHistoric($cuser->id, 'historic', $pagenum);
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function storeJobEmail($data)
    {
        ...
        $job = $this->repository->findOrFail($data['user_email_job_id']);
        ...
        $job = $this->repository->update($job->id, $job);
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $job_detail = $this->repository->with(['translatorJobRel'])->find($jobid);
        ...
        $this->translatorRepository->update($tr->id, $tr);
    ...
    {
        $user_meta = $this->userMetaRepository->findByUserId($user_id);
        ...
        $languages = $this->userLanguageRepository->findByUserId($user_id);
        $userlanguage = collect($languages)->pluck('lang_id')->all();
        $gender = $user_meta->gender;
        $translator_level = $user_meta->translator_level;
        $job_ids = $this->repository->getJobs($user_id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
        foreach ($job_ids as $k => $v)     // checking translator town
        {
            $job = $this->repository->find($v->id);
            $jobuserid = $job->user_id;
            $checktown = $this->repository->checkTowns($jobuserid, $user_id);
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        $jobPosterMeta = $this->$userMetaRepository->findByUserId($job->user_id);
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $users = $this->userRepository->getPotentialUsers($translator_type, $joblanguage, $gender, $translator_level, $translatorsId);
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout'])) {
        ...
        $this->repository->update($job->id, $job);
        return true;
//        }
        return false;
    }
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $translator = $this->repository->getJobsAssignedTranslatorDetail($job);
...
//        }
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        if ($this->isNeedToSendPush($user->id)) {
            ...
            $this->sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $msg_text, $this->repository->isNeedToDelayPush($user->id));
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if (!is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            if (!is_null($current_translator) && ((isset($data['translator']) && $current_translator->user_id != $data['translator']) || $data['translator_email'] != '') && (isset($data['translator']) && $data['translator'] != 0)) {
                if ($data['translator_email'] != '') $data['translator'] = $this->userRepository->findByEmail($data['translator_email'])->id;
                $new_translator = $current_translator->toArray();
                $new_translator['user_id'] = $data['translator'];
                unset($new_translator['id']);
                $new_translator = $this->translatorRepository->create($new_translator);
                $current_translator->cancel_at = Carbon::now();

                $current_translator = $this->translatorRepository->update($new_translator->id, $new_translator);
                ...
                if ($data['translator_email'] != '') $data['translator'] = $this->userRepository->findByEmail($data['translator_email'])->id;
                $new_translator = $this->translatorRepository->create(['user_id' => $data['translator'], 'job_id' => $job->id]);
                $log_data[] = [
                    'old_translator' => null,
                    'new_translator' => $new_translator->user->email
                ];
    ...
    {
        $job = $this->repository->findOrFail($job_id);
    ...
    public function acceptJob($data, $user)
    {
        ...
        $job_id = $data['job_id'];
        $job = $this->repository->findOrFail($job_id);
        if (!$this->repository->isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if (!$this->repository->isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && $this->repository->insertTranslatorJobRel($cuser->id, $job_id)) {
                $job->status = 'assigned';
                $job->save();
                $user = $job->user()->get()->first();
                $mailer = new AppMailer();
...
//                Event::fire(new JobWasCanceled($job));
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        $languages =$this->$userLanguageRepository->findByUserId($cuser->id);
        $userlanguage = collect($languages)->pluck('lang_id')->all();
        ...
        $job_ids = $this->repository->getJobs($cuser->id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
        foreach ($job_ids as $k => $job) {
            $jobuserid = $job->user_id;
            $job->specific_job = $this->repository->assignedToPaticularTranslator($cuser->id, $job->id);
            $job->check_particular_job = $this->repository->checkParticularJob($cuser->id, $job);
            $checktown = $this->repository->checkTowns($jobuserid, $cuser->id);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function endJob($post_data)
    {
        $completeddate = date('Y-m-d H:i:s');
        $jobid = $post_data["job_id"];
        $job_detail = $this->repository->query()->with('translatorJobRel')->find($jobid);
    ...
    public function getAll(array $requestdata, $limit = null, $cuser)
    {
        $consumer_type = $cuser->consumer_type;

        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            $allJobs = $this->repository->query();
            ...
            if (isset($requestdata['customer_email']) && count($requestdata['customer_email']) && $requestdata['customer_email'] != '') {
                $users = $this->$userRepository->findWhereInEmail($requestdata['customer_email']);
                if ($users) {
    ...
    public function alerts(array $requestdata, $authUser)
    {
        ...
        $languages = $this->languageRepository->where('active', '1')->orderBy('language')->get();
        
        $all_customers = $this->userRepository->findByUserType(1)->lists('email');
        $all_translators = $this->userRepository->findByUserType(2)->lists('email');
        ...
        if ($cuser && $cuser->is('superadmin')) {
            ...
            if (isset($requestdata['customer_email']) && $requestdata['customer_email'] != '') {
                $user = $this->userRepository->findByEmail($requestdata['customer_email']);
                if ($user) {
    ...
    {
        $throttles = $this->throttleRepository->where('ignore', 0)->with('user')->paginate(15);

        return ['throttles' => $throttles];
    }

    public function bookingExpireNoAccepted(array $requestdata, $authUser)
    {
        $languages = $this->languageRepository->where('active', '1')->orderBy('language')->get();
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if (isset($requestdata['translator_email']) && $requestdata['translator_email'] != '') {
                $user = $this->userRepository->findByEmail($requestdata['translator_email']);
                if ($user) {
                    $allJobIDs = DB::table('translator_job_rel')->where('user_id', $user->id)->lists('job_id');
                    $allJobs->whereIn('jobs.id', $allJobIDs)
                        ->where('jobs.status', 'pending')
    ...
    {
        $throttle = $this->throttleRepository->find($id);
        $throttle->ignore = 1;
        $this->throttleRepository->update($throttle->id, $throttle);
        return ['success', 'Changes saved'];
    }
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
        ...
        if ($job['status'] != 'timedout') {
            $affectedRows = $this->repository->update($jobid, $datareopen);
            $new_jobid = $jobid;
        } else {
            $job['status'] = 'pending';
            ...
            $affectedRows = $this->repository->create($job);
